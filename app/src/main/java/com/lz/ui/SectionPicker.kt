...
fun SectionPropertiesGrid(section: SectionProfile) {
    ...
    Column(
        ...
        Row(modifier = Modifier.fillMaxWidth()) {
            PropertyItem(
                "Area",
                UnitFormattingService.formatArea(section.area, system),
                Modifier.weight(1f)
            )
            PropertyItem(
                "Depth (d)",
                UnitFormattingService.formatSmallLength(section.depth, system),
                Modifier.weight(1f)
            )
        }
        Row(modifier = Modifier.fillMaxWidth()) {
            PropertyItem(
                "Ix",
                UnitFormattingService.formatInertia(section.propertiesStrongAxis.i, system),
                Modifier.weight(1f)
            )
            PropertyItem(
                "Sx",
                UnitFormattingService.formatSectionModulus(section.propertiesStrongAxis.s, system),
                Modifier.weight(1f)
            )
        }
        Row(modifier = Modifier.fillMaxWidth()) {
            PropertyItem(
                "Iy",
                UnitFormattingService.formatInertia(section.propertiesWeakAxis.i, system),
                Modifier.weight(1f)
            )
            PropertyItem(
                "Sy",
                UnitFormattingService.formatSectionModulus(section.propertiesWeakAxis.s, system),
                Modifier.weight(1f)
            )
        }
        ...
        if (section is SteelProfile) {
            Row(modifier = Modifier.fillMaxWidth()) {
                PropertyItem(
                    "bf",
                    UnitFormattingService.formatSmallLength(section.flangeWidth, system),
                    Modifier.weight(1f)
                )
                PropertyItem(
                    "tf",
                    UnitFormattingService.formatSmallLength(section.flangeThickness, system),
                    Modifier.weight(1f)
                )
            }
            Row(modifier = Modifier.fillMaxWidth()) {
                PropertyItem(
                    "tw",
                    UnitFormattingService.formatSmallLength(section.webThickness, system),
                    Modifier.weight(1f)
                )
                PropertyItem(
                    "J",
                    String.format(Locale.US, "%.2f in⁴", section.torsionalConstantJ),
                    Modifier.weight(1f)
                )
            }
        }
...
@Composable
fun PropertyItem(label: String, value: String, modifier: Modifier = Modifier.Companion) {
