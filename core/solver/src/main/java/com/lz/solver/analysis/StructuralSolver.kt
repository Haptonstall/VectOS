...
class StructuralSystem {
    ...
    private val springDofs = mutableMapOf<Pair<Int, DofType>, Double>()
    ...
    fun addElement(
        ...
        e: Double, i: Double, iy: Double,
        g: Double, a: Double, j: Double,
        dofConfig: DofConfig
    ) {
     ...
     * This should be the NEGATIVE of the Fixed End Actions (FEA).
     */
    ...
    /** Fully restrains a DOF (zero displacement). Uses penalty method internally. */
    fun fixNode(nodeIdx: Int, dof: DofType) {
        fixedDofs.add(nodeIdx to dof)
    }

    /**
     * Adds a linear spring at a DOF with the given stiffness (kip/in or kip·in/rad).
     * The spring stiffness is added directly to the global K diagonal — no penalty needed.
     */
    fun addSpringNode(nodeIdx: Int, dof: DofType, stiffness: Double) {
        val key = nodeIdx to dof
        springDofs[key] = (springDofs[key] ?: 0.0) + stiffness
    }

    fun addNodeForce(nodeIdx: Int, dof: DofType, value: Double) {
        val key = nodeIdx to dof
        externalForces[key] = (externalForces[key] ?: 0.0) + value
    }
    ...
    fun getSpringDofs()             = springDofs.toMap()
    fun getExternalForces()         = externalForces.toMap()

    fun getNodeDofs(nodeIdx: Int): List<DofType> = DofType.values().toList()
}
...
data class DofConfig(
    ...
    companion object {
        val SIX_DOF = DofConfig(
            6, listOf(DofType.UX, DofType.UY, DofType.UZ, DofType.RX, DofType.RY, DofType.RZ)
        )
    }
}
...
class StructuralSolver {
    ...
    fun solve(system: StructuralSystem): StructuralResult {
        ...
        val springDofs    = system.getSpringDofs()
        ...
        // --- 1. Assemble global stiffness and equivalent nodal loads ---
        elements.forEach { el ->
            val n1 = nodes[el.startNode]
            val n2 = nodes[el.endNode]
            val dx = n2.x - n1.x; val dy = n2.y - n1.y; val dz = n2.z - n1.z
            ...
            val dofIdx   = getElementDofIndices(el)
            ...
            for (i in 0 until 12) {
                for (j in 0 until 12) {
                    globalK[dofIdx[i]][dofIdx[j]] += keGlobal[i][j]
                }
                globalF[dofIdx[i]] += feqGlobal[i]
            }
        }

        // --- 2. Assemble external load vector ---
        externalForces.forEach { (key, value) ->
            val (nodeIdx, dof) = key
            globalF[nodeIdx * dofsPerNode + dof.ordinal] += value
        }

        // --- 3. Apply spring boundary conditions (direct stiffness addition) ---
        // Springs add their stiffness directly to the K diagonal. No penalty needed —
        // the spring stiffness IS the physical restraint stiffness.
        springDofs.forEach { (key, k_spring) ->
            val (nodeIdx, dof) = key
            val globalDof = nodeIdx * dofsPerNode + dof.ordinal
            globalK[globalDof][globalDof] += k_spring
        }

        // --- 4. Apply fixed boundary conditions (penalty method) ---
        // Penalty >> max diagonal stiffness term to enforce zero displacement.
        // We compute the penalty after springs are added to keep it numerically sound.
        val maxDiag = (0 until totalDofs).maxOfOrNull { globalK[it][it] } ?: 1.0
        val penalty = maxDiag * 1e10

        fixedDofs.forEach { (nodeIdx, dof) ->
            val globalDof = nodeIdx * dofsPerNode + dof.ordinal
            globalK[globalDof][globalDof] += penalty
            // RHS remains zero: enforces u = 0 at this DOF
        }

        // --- 5. Solve [K]{u} = {F} ---
        val u = solveLinearSystem(globalK, globalF)

        // --- 6. Post-process displacements and reactions ---
        ...
        nodes.forEach { node ->
            ...
            DofType.values().forEach { dof ->
                ...
                val disp = u[globalDof]
                displacements[node.id to dof] = disp

                val isFixed  = fixedDofs.contains(node.id to dof)
                val kSpring  = springDofs[node.id to dof]

                reactions[node.id]!![dof] = when {
                    isFixed  -> -disp * penalty         // penalty reaction
                    kSpring != null -> -disp * kSpring  // spring reaction = k * u
                    else     -> 0.0
                }
            }
        }

        // --- 7. Calculate element end forces (local frame) ---
        ...
        elements.forEach { el ->
            ...
            val ueGlobal  = DoubleArray(12) { u[dofIdx[it]] }
            val ueLocal   = transformVector(ueGlobal, R, transpose = false)
            val feqLocal  = eqForces[el.id] ?: DoubleArray(12)

            // f_local = k_local * u_local - feq_local
            val feLocal = DoubleArray(12) { i ->
                var sum = 0.0
                for (j in 0 until 12) sum += keLocal[i][j] * ueLocal[j]
                sum - feqLocal[i]
            }
    ...
    private fun calculateRotationMatrix(
        n1: StructuralSystem.Node,
        n2: StructuralSystem.Node
    ): Array<DoubleArray> {
        val dx = n2.x - n1.x; val dy = n2.y - n1.y; val dz = n2.z - n1.z
        val L  = sqrt(dx * dx + dy * dy + dz * dz)
        if (L < 1e-9) return Array(3) { i -> DoubleArray(3) { j -> if (i == j) 1.0 else 0.0 } }
        val cx = dx / L; val cy = dy / L; val cz = dz / L
        val R = Array(3) { DoubleArray(3) }
        if (abs(cx) < 1e-6 && abs(cz) < 1e-6) {
            // Member is vertical (aligned with Y)
    ...
    private fun transformMatrix(kLocal: Array<DoubleArray>, R: Array<DoubleArray>): Array<DoubleArray> {
        ...
        for (i in 0 until 4) {
            val o = i * 3
            for (r in 0 until 3) for (c in 0 until 3) T[o + r][o + c] = R[r][c]
        }
        val TT   = transpose(T)
    ...
    private fun transformVector(v: DoubleArray, R: Array<DoubleArray>, transpose: Boolean): DoubleArray {
        ...
        for (i in 0 until 4) {
            val sub = doubleArrayOf(v[i*3], v[i*3+1], v[i*3+2])
            val out = multiplyVec(mat, sub)
            res[i*3] = out[0]; res[i*3+1] = out[1]; res[i*3+2] = out[2]
        }
        return res
    }
    ...
    private fun transpose(A: Array<DoubleArray>): Array<DoubleArray> {
        ...
        return Array(m) { j -> DoubleArray(n) { i -> A[i][j] } }
    }
    ...
    private fun getElementDofIndices(el: StructuralSystem.Element): IntArray {
        val idx = IntArray(12)
        val n1b = el.startNode * 6; val n2b = el.endNode * 6
        for (i in 0 until 6) { idx[i] = n1b + i; idx[i + 6] = n2b + i }
        return idx
    }
    ...
    private fun calculateElementStiffness(el: StructuralSystem.Element, L: Double): Array<DoubleArray> {
        ...
        // Torsion (RX)
        val kT = G * J / L
        k[3][3] = kT; k[3][9] = -kT; k[9][3] = -kT; k[9][9] = kT

        // Strong-axis bending (UY, RZ)
        val k1z = 12*E*I / L.pow(3); val k2z = 6*E*I / L.pow(2)
        val k3z =  4*E*I / L;        val k4z = 2*E*I / L
        ...
        // Weak-axis bending (UZ, RY) — sign convention: θ_y = -dw/dx
        val k1y = 12*E*Iy / L.pow(3); val k2y = 6*E*Iy / L.pow(2)
        val k3y =  4*E*Iy / L;        val k4y = 2*E*Iy / L
    ...
    private fun solveLinearSystem(A: Array<DoubleArray>, b: DoubleArray): DoubleArray {
        ...
        for (i in 0 until n) {
            var maxVal = abs(A[i][i]); var maxRow = i
            for (k in i + 1 until n) {
                if (abs(A[k][i]) > maxVal) { maxVal = abs(A[k][i]); maxRow = k }
            }
            if (abs(A[maxRow][i]) < 1e-18) continue // singular/under-constrained column
            ...
            for (k in i + 1 until n) {
                val c = -A[k][i] / A[i][i]
                for (j in i until n) A[k][j] = if (i == j) 0.0 else A[k][j] + c * A[i][j]
                b[k] += c * b[i]
            }
        }
        ...
        for (i in n - 1 downTo 0) {
            if (abs(A[i][i]) < 1e-18) continue
            x[i] = b[i] / A[i][i]
            for (k in i - 1 downTo 0) b[k] -= A[k][i] * x[i]
        }
        return x
    }
}